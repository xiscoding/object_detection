...
def show_array(frame: np.ndarray):
    """
    Display array `frame` in a window. `frame` is expected to have data in BGR order.
    """
    import cv2
    window_name = "Image"
...
class NotebookAlert(Exception):
    def __init__(self, message: str, alert_class: str):
        """
        Print a message with a given alert class.
    ...
    def show_message(self):
        print(f"[{self.alert_class.upper()}] {self.message}")
